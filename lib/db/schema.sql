-- سُفرة — السكيما الأساس. القسم ٦ من المواصفة، وقيود القسم ٥ مفروضة هنا لا في التوثيق.
--
-- ⚠ هذا الملف **مُجمَّد**. لا تعدّله.
--
-- `CREATE TABLE IF NOT EXISTS` لا يفعل شيئاً على قاعدة قائمة: الجدول الجديد يُنشأ،
-- أما العمود الجديد أو CHECK الموسَّع فيُهمَل بصمت. أي تعديل هنا سيعمل على قاعدة
-- التطوير النظيفة ولن يصل الإنتاج أبداً — وهذا أسوأ من الفشل الصريح.
--
-- كل تغيير لاحق ملف مرقَّم في lib/db/migrations/ يطبّقه lib/db/schema.ts.
--
-- اصطلاحان ثابتان:
